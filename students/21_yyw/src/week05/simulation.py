...
def print_covariance_matrices(cov_empirical, cov_theoretical, rho):
    ...
    print(
        f"Corr(β̂₁, β̂₂) = {cov_empirical[0, 1] / np.sqrt(cov_empirical[0, 0] * cov_empirical[1, 1]):.6f}"
    )
    ...
    print(
        f"Corr(β̂₁, β̂₂) = {cov_theoretical[0, 1] / np.sqrt(cov_theoretical[0, 0] * cov_theoretical[1, 1]):.6f}"
    )
