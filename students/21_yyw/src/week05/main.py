...
def main():
    ...
    print(
        f"  Corr(β̂₁, β̂₂): {results_orth['beta1_hat'].corr(results_orth['beta2_hat']):.6f}"
    )
    ...
    print(f"\n共线性场景统计:")
    print(
        f"  β̂₁ 均值: {results_collinear['beta1_hat'].mean():.6f} (理论: {TRUE_BETA[0]})"
    )
    print(
        f"  β̂₂ 均值: {results_collinear['beta2_hat'].mean():.6f} (理论: {TRUE_BETA[1]})"
    )
    print(f"  β̂₁ 方差: {results_collinear['beta1_hat'].var():.6f}")
    print(f"  β̂₂ 方差: {results_collinear['beta2_hat'].var():.6f}")
    print(
        f"  Corr(β̂₁, β̂₂): {results_collinear['beta1_hat'].corr(results_collinear['beta2_hat']):.6f}"
    )
