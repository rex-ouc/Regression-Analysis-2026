...
def analysis(df_results, true_params):
    ...
    # 左图：β₀ 的分布
    axes[0].hist(
        df_results["ols_beta_0"], bins=30, alpha=0.7, label="statsmodels", density=True
    )
    axes[0].hist(
        df_results["sklearn_beta_0"], bins=30, alpha=0.5, label="sklearn", density=True
    )
    axes[0].axvline(
        beta_0_true,
        color="r",
    ...
    # 右图：β₁ 的分布
    axes[1].hist(
        df_results["ols_beta_1"], bins=30, alpha=0.7, label="statsmodels", density=True
    )
    axes[1].hist(
        df_results["sklearn_beta_1"], bins=30, alpha=0.5, label="sklearn", density=True
    )
    axes[1].axvline(
        beta_1_true,
        color="r",
